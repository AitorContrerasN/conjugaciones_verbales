...
# Tu HTML como una string; asegúrate de que el HTML está completo y bien formado
...
for block in conj_blocks:
    # Itera a través de todos los nodos directos del bloque
    for child in block.children:
        # Si el hijo es un NavigableString y no es solo espacio en blanco, captúralo
        if isinstance(child, NavigableString):
            text = child.strip()
            if text:
                conjugations.append(text)
        # Si el hijo es un <br>, mira el siguiente hermano NavigableString
        elif child.name == 'br':
...
# Imprime todas las conjugaciones encontradas
